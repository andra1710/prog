...
public class epsilon{
	...
	static void Main() {
		int i = 0;
		while(i+1>i) {i++;}
		WriteLine($"My max int using while loop = {i}");
		WriteLine($"My max int from int.MaxValue = {int.MaxValue}");

		int j = 0;
		while(j-1<j) {j--;}
		WriteLine($"My min int using while loop)= {j}");
		WriteLine($"My min int from int.MaxValue = {int.MinValue}");
